...
uint32_t SinFre[4] = {1000000, 3000, 1000, 1000};
uint32_t SinAmp[4] = {1023, 1023, 65, 1023};		//1023对应50欧姆示波器的峰值125mv 峰峰值250mv 400对应峰峰值100mv 
...
void Init_AD9959(void)
{
    ...
    uint8_t FR1_DATA[3] = {0xAB, 0x00, 0x00}; // 10 frequency doubling
    ...
    AD9959_Write_Frequence(0, SinFre[0]);
    AD9959_Write_Frequence(1, SinFre[1]);
    AD9959_Write_Frequence(2, SinFre[2]);
    AD9959_Write_Frequence(3, SinFre[3]);

    AD9959_Write_Amplitude(3, SinAmp[3]);
    AD9959_Write_Amplitude(0, SinAmp[0]);
    AD9959_Write_Amplitude(1, SinAmp[1]);
    AD9959_Write_Amplitude(2, SinAmp[2]);
}
...
void InitIO_9959(void)
{
  ...
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
	...
	GPIO_InitStruct.Pin = SDIO1_Pin;
	HAL_GPIO_Init(SDIO1_GPIO_Port, &GPIO_InitStruct);	
	
	GPIO_InitStruct.Pin = SDIO2_Pin;
	HAL_GPIO_Init(SDIO2_GPIO_Port, &GPIO_InitStruct);	
	
	GPIO_InitStruct.Pin = SDIO3_Pin;
	HAL_GPIO_Init(SDIO3_GPIO_Port, &GPIO_InitStruct);		
	...
	GPIO_InitStruct.Pin = PS3_Pin;
	HAL_GPIO_Init(PS3_GPIO_Port, &GPIO_InitStruct);
...
//    WRT(SCLK, 0);
	HAL_SPI_Transmit_DMA(&hspi1, &ControlValue, 1);
    // Write data
    // 每个字节的数据都是从高位开始传输
    // 第一个传输的字节是最高位的字节
...
//        WRT(SCLK, 0);
//    }
		HAL_SPI_Transmit_DMA(&hspi1, RegisterData,NumberofRegisters);
...
void AD9959_Sweep_Frequency(uint8_t Channel, uint32_t Start_Freq, uint32_t Stop_Freq, uint32_t Step, uint32_t time, uint8_t NO_DWELL)
{
    ...
    uint8_t Time_word;
    Start_Freq_word = (uint32_t)Start_Freq * FreqUnit;
    Stop_Freq_Word = (uint32_t)Stop_Freq * FreqUnit;
    Step_Word = (uint32_t)Step * FreqUnit;
...
uint32_t Get_Freq(void)
{
    ...
    int Freq = (data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3]) / FreqUnit;
    return Freq;
}
...
void Freq2Word(double f, uint8_t *fWord)
{
    // fWord 4 bytes
    uint32_t Temp;
    Temp = (uint32_t)f * FreqUnit; // The input frequency factor is divided into four bytes.  FreqUnit=(2^32)/500000000
